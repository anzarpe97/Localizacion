...
def access_export_user_groups(env):
    """Export user groups - Odoo 18 compatible signature"""
    export_id = env["ir.exports"].search([("name", "=", "Default Template")], limit=1)
    if not export_id:
        export_id = env["ir.exports"].create(
    ...
    fields_data = fields_info(
        env, "res.groups", [f["name"] for f in export_fields_list]
    )
    fields_list = [
...
def access_post_install_report_action_hook(env):
    """Post-install hook - Odoo 18 compatible signature"""
    actions = env["ir.actions.actions"].search([])
    for action in actions:
        env["report.action.data"].create(
            {"name": action.name, "access_action_id": action.id}
        )
