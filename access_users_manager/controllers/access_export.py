...
class InheritExportXlsxWriter(ExportXlsxWriter):
    def __init__(self, field_names, row_count=0, env=None):
        ...
        self.columns_headers = field_names  # Required by parent class in Odoo 18
        ...
        if env:
            decimal_places = [
                res["decimal_places"]
                for res in env["res.currency"].search_read([], ["decimal_places"])
            ]
        else:
            decimal_places = [
                res["decimal_places"]
                for res in request.env["res.currency"].search_read(
                    [], ["decimal_places"]
                )
            ]
