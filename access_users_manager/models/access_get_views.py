...
class BaseModel(models.AbstractModel):
    ...
    def get_view(self, view_id=None, view_type="form", **options):
        ...
        lst = [int(x) for x in cids.split(",") if x.isdigit()]
    ...
    def get_view(self, view_id=None, view_type="form", **options):
        ...
        if view_type == "form":
            ...
            if hide_field_access:
                for field in hide_field_access.mapped("access_field_id"):
                    if field.ttype in ["many2many", "many2one"]:
                        for field_ele in doc.xpath(
                            "//field[@name='" + field.name + "']"
                        ):
                            options = (
                                "options" in field_ele.attrib.keys()
                                and field_ele.attrib["options"]
                                or "{}"
                            )
                            options = ast.literal_eval(options)
                            options.update(
        ...
        if view_type == "kanban":
            ...
            for button in hide_button_ids:
                ...
                for link in button.access_kanban_button_ids:
                    ...
                    for ele in element:
                        if (
                            not ele.text.startswith("\n")
                            and ele.text == link.access_tab_button_string
                        ) or ele.text.startswith("\n"):
                            ele.attrib.update({"class": "d-none"})
                for link in button.access_kanban_button_ids:
