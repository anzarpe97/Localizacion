...
class ResGroups(models.Model):
    ...
    access_profiles_count = fields.Integer(
        "# Perfiles", compute="_compute_profiles_count"
    )
        ...
        if self._context.get("install_filename") or self._context.get(
            ...
            for (
                ...
                if app.xml_id == "base.module_category_user_type":
                    ...
                    # application name with a selection field
                    field_name = name_selection_groups(gs.ids)
                    # Odoo 18: use readonly instead of attrs
                    if user_type_field_name:
                        attrs["readonly"] = (
                            f"{user_type_field_name} != {group_employee.id}"
                        )
                    attrs["on_change"] = "1"
                    ...
                    for g in gs:
                        ...
                        if g == group_no_one:
                            ...
                            dest_group.append(
                                E.field(name=field_name, invisible="1", **field_attrs)
                            )
                        else:
                            dest_group.append(E.field(name=field_name, **field_attrs))
                        group_count += 1
            ...
            if user_type_field_name:
                # Odoo 18: use invisible instead of attrs
                user_type_invisible = f"{user_type_field_name} != {group_employee.id}"
            else:
                user_type_invisible = None
            ...
            # Odoo 18: use invisible instead of attrs
            group2_attrs = (
                {"invisible": user_type_invisible} if user_type_invisible else {}
            )
            group3_attrs = (
                {"invisible": user_type_invisible} if user_type_invisible else {}
            )
            group4_attrs = (
                {"invisible": user_type_invisible, "groups": "base.group_no_one"}
                if user_type_invisible
                else {"groups": "base.group_no_one"}
            )

            xml = E.field(
                *(xml0),
                E.group(*(xml1), groups="base.group_no_one"),
                E.group(*(xml2), **group2_attrs),
                E.group(*(xml3), **group3_attrs),
                E.group(*(xml4), **group4_attrs),
                name="groups_id",
                position="replace",
            )
    ...
    def access_export_user_groups(self):
        export_id = (
            self.env["ir.exports"]
            .sudo()
            .search([("name", "=", "Default Template")], limit=1)
        )
        if not export_id:
            export_id = self.env["ir.exports"].create(
                {"name": "Default Template", "resource": "res.groups"}
            )
            export_fields = [
                "id",
                "name",
                ...
                "view_access/id",
                "model_access/name",
                "model_access/perm_write",
                "model_access/perm_read",
                "rule_groups/id",
            ]
            for field in export_fields:
                self.env["ir.exports.line"].create(
                    {"name": field, "export_id": export_id.id}
                )
