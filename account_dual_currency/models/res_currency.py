        ...
        for rec in self:
            ...
            if facturas:
                for f in facturas:
                    ...
                    for l in f.line_ids:
                        l.tax_today = rec.env['res.currency'].get_trm_systray()
                        l._debit_usd()
                        l._credit_usd()
                    for d in f.invoice_line_ids:
                        d.tax_today = rec.env['res.currency'].get_trm_systray()
                        d._price_unit_usd()
        ...
        for rec in self:
            ...
            for lp in list_product_ids:
                ...
                for p in product_id_bs:
                    p.fixed_price = lp.fixed_price * div
