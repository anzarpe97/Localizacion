...
class AccountReport(models.Model):
    ...
    def format_value(self, options, value, currency=False, blank_if_zero=False, figure_type=None, digits=1):
        ...
        # Loggear el tipo de figura
        _logger.info("Figure type: %s", figure_type) 
        # Loggear el tipo de figura
        _logger.info("options type: %s", options) 
        _logger.info("currency type: %s", currency) 
...
#                 user_currency = user_company.currency_id
#                 currency = None
#                 # Si hay un report_id, verificamos si tiene una moneda personalizada
#                 if report_id:
#                     report = self.env['account.report'].browse(report_id)
#                     if report.curr_currency_id:
#                         user_currency = report.curr_currency_id

#                 # Asignamos la moneda en función de las condiciones
#                 if user_currency:
#                     currency = user_currency
#                 else:
#                     currency = self.env['res.currency'].browse(query_res['currency_id'][0])

#                 expected_date = len(query_res['expected_date']) == 1 and query_res['expected_date'][0] or \
#                                 len(query_res['due_date']) == 1 and query_res['due_date'][0]
#                 _logger.error(user_currency)
#                 # Actualizamos el diccionario rslt con los nuevos valores
#                 rslt.update({
...
#                     'currency_id': currency,   #query_res['currency_id'][0] if len(query_res['currency_id']) == 1 else None,
