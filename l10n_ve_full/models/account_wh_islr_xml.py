...
class IslrXmlWhDoc(models.Model):
    ...
    def _xml(self):
        ...
        for ixwd_id in self.ids:
            ...
            if local_ids:
                sql = '''
                SELECT partner_vat, correlative as control_number, porcent_rete,
                    concept_code,invoice_number,
                    SUM(COALESCE(base,0)) as base, account_invoice_id, date_ret
                FROM account_wh_islr_xml_line
                WHERE id in (%s)
                GROUP BY partner_vat, correlative, porcent_rete, concept_code,
            ...
            for line in xml_lines:
                partner_vat, correlative, porcent_rete, concept_code, \
                invoice_number, base, inv_id, date_ret = line
                correlative = correlative.replace("-", "")
                ...
                SubElement(detalle, "NumeroControl").text = correlative if len(correlative) < 11 else correlative[-10:]
...
class IslrXmlWhLine(models.Model):
    ...
    @api.depends('account_invoice_id.correlative')
    def _compute_nro_ctrl(self):
        for record in self:
            record.nro_ctrl = record.account_invoice_id.correlative or "NA"

    nro_ctrl = fields.Char(string="Número de Control", compute="_compute_nro_ctrl", store=True)
    correlative = fields.Char(
        string="Correlative",
        related="account_invoice_id.correlative",
        store=True
    )
    ...
    def _compute_control_number(self):
        for record in self:
            record.control_number = record.account_invoice_id.correlative or "NA"
