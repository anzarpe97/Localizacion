...
class ResPartner(models.Model):
    ...
    def create(self, vals_list):
        for val in vals_list:
            ident = val.get("identification_id")
            if 'vat' in val and val['vat']:
                if val['company_type'] == 'person':
                    val['identification_id'] = val['vat']
                else:
                    val['rif'] = val['vat']
            else:
                if 'identification_id' in val and val['identification_id']:
                    val['vat'] = val['identification_id']
            
            if 'company_type' in val and val['company_type'] == 'person':
                if 'identification_id' in val and val['identification_id'] and 'nationality' in val and val['nationality']:
                    valor = ident
                    nationality = val['nationality']
                    self.validation_document_ident(valor, nationality)
                    if 'rif' in val and val['rif']:
                        ...
                        if self.validate_rif_duplicate(val.get('rif'), self):
                            raise UserError('El cliente o proveedor ya se encuentra registrado con el rif: %s y se encuentra activo' % (val.get('rif')))
                    if not self.validate_ci_duplicate(val.get('identification_id', False), True):
                        raise UserError('El cliente o proveedor ya se encuentra registrado con el Documento: %s' % (val.get('identification_id', False)))
                    val['identification_id'] = ident

            if 'company_type' in val and val['company_type'] == 'company':
                res_country_id = self.env['res.country'].browse(val['country_id'])
                if res_country_id.code == 'VE':
                    if not self.validate_rif_er(val.get('rif')):
                        raise UserError('El rif tiene el formato incorrecto. Ej: V-01234567-8, E-01234567-8, J-01234567-8 o G-01234567-8. Por favor verifique el formato y si posee los 12 caracteres como se indica en el Ej. e intente de nuevo')
                    if self.validate_rif_duplicate(val.get('rif'), self):
                        raise UserError('El cliente o proveedor ya se encuentra registrado con el rif: %s y se encuentra activo' % (val.get('rif')))
